...
program RBC
    ...
    implicit none
    integer :: pisize , etasize
    integer :: i , j , ii , jj , etai
    ! start timer
    !call tic()
    ...
    close(1)
    open(2, file='eta.dat', status='new')
    do ii =1,etasize
    write(2,*) eta(ii)
    end do
    close (2)
    ...
    open(3,file = 'k_before.dat',status = 'new')
    i = size(l)
    write(*,*) i
...
call grid_Cons_Equi(k, k_l, k_u)
    ...
    open(3,file = 'k_after.dat',status = 'new')
    i = size(k)
    write(3,*) k
    close (3)
    ...
    open(4,file = 'coeff_r.dat',status = 'new')
    write(*,*) size(coeff_r)
    write(4,*) coeff_r
    close (4)
...
contains
    ...
    subroutine output()
        ...
        call simulate_economy()
        open(5,file = 'simulation_is_t.dat',status = 'new')
        open(6,file = 'simulation_eta.dat',status = 'new')
        open(7,file = 'simulation_k.dat',status = 'new')
        open(8,file = 'simulation_c.dat',status = 'new')
        open(9,file = 'simulation_l.dat',status = 'new')
        open(10,file = 'simulation_y.dat',status = 'new')
        open(11,file = 'simulation_i.dat',status = 'new')
        open(12,file = 'simulation_r.dat',status = 'new')
        open(13,file = 'simulation_w.dat',status = 'new')
